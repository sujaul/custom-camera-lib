...
public abstract class BaseChareemActivity<CameraId> extends ChareemCameraActivity<CameraId>
    ...
    private void extractConfiguration(Bundle bundle) {
        if (bundle != null) {
            ...
            if (bundle.containsKey(CameraConfiguration.Arguments.CAMERA_FACING_TYPE))
                currentCameraType = bundle.getInt(CameraConfiguration.Arguments.CAMERA_FACING_TYPE);
    ...
    private void startPreviewActivity() {
        try {
            if (getMediaAction() == CameraConfiguration.MEDIA_ACTION_PHOTO || getCameraController().getOutputFile().toString().contains(".jpg")){
                Bitmap rotatedBitmap = Utils.rotateImageIfRequired(getCameraController().getOutputFile().toString(), this);
